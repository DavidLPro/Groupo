    ...
    {!isLoading ? (<>
        <div className='login'>
            ...
            <form onSubmit={this.postLogin} disabled={validForm}>
                ...
                <input type='submit' disabled={validForm} label='Connexion' value='Se Connecter' />
            </form>
        </div>

        <div className="sign">
            <h3>Pas encore inscrit ?</h3>
            <button value={'Register'} onClick={this.props.navigateTo} >S'INSCRIRE</button>
        </div>
    </>) : (<TinyLoader />)}
...
class Register extends React.Component {
  ...
  async postLogin(event) {
    ...
    .catch((err) => {
      console.error('Error Login request!');
      console.log(err);
      this.setState({ userLogged: null, validForm: false, inAction: false, error: 'Adresse Email existante. Essayer une autre adresse' });
    });
  }
    ...
    {!isLoading ? (<>
      <div className='login'>
        ...
        <label htmlFor="Nom">Nom : </label>
          ...
          <input id='Prenom' name='firstname' type='text' 
